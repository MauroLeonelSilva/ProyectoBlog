...
class ModelBase(models.Model):
	id = models.UUIDField(default=uuid.uuid4, primary_key=True, db_index=True, editable=False) #id unica universal creada con shell "319118fa-8987-432d-b59d-ef004026445d"
...
class CanalManager(models.Manager):
	...
	def obtener_o_crear_canal_usuario_actual(self, user):
		...
		if qs.exists():
			return qs.order_by("tiempo").first, False #el false es para no crearlo en la BD
	...
	def obtener_o_crear_canal_ms(self, username_a, username_b):
		qs = self.filtrar_ms_por_privado(username_a, username_b)  #aca filtramos si hay una conversacion si la hay lo devuelve
		if qs.exists():

			return qs.order_by("tiempo").first(), False #devuelve el objeto si fue creado o no
		...
		CanalUsuario.objects.bulk_create([canal_usuario_a, canal_usuario_b]) #bulk es un metodo de administrador de objetos usando una matriz
